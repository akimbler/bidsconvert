...
def intended_for_gen(fmap_nifti, niftis):
    ...
    for num in sorted([x for x in out_dict]):
        ...
        acq_found = any(
            [fmap_entities["acquisition"] != ent["datatype"] for ent in target_entities]
        )
        ...
        if target_entities[0]["datatype"] == "fmap":
            thing_found = any(
                [
                    all([fmap_entities[x] == i[x] for x in fmap_entities if x != "run"])
        ...
        if "session" in target_entities[0]:
            intended_for.extend(
                sorted(
                    [
                        op.join(
                            "ses-{0}".format(target_entities[0]["session"]),
                            target_entities[0]["datatype"],
                            x.filename,
                        )
                        for x in out_dict[num]
                    ]
                )
            )
        else:
            intended_for.extend(
                sorted(
                    [
                        op.join(target_entities[0]["datatype"], x.filename)
                        for x in out_dict[num]
                    ]
                )
            )
...
def complete_jsons(bids_dir, subs, ses, overwrite):
    ...
    for sid in subs:
        if ses:
            ...
            niftis = layout.get(
                subject=sid, extension="nii.gz", datatype=["func", "fmap", "dwi"]
            )
        for nifti in niftis:
            ...
            if ("EffectiveEchoSpacing" in data.keys()) and (
                ...
                etl = nib.load(nifti.path).shape[pe_idx] // float(
                    data.get("ParallelReductionFactorInPlane", 1.0)
                )
                ees = data.get("EffectiveEchoSpacing", None)
                if ees is None:
                    raise Exception('Field "EffectiveEchoSpacing" not ' "found in json")
                data["TotalReadoutTime"] = ees * (etl - 1)
                dump = 1
            if "task" in nifti.get_entities() and (
                overwrite or "TaskName" not in data.keys()
            ):
                data["TaskName"] = nifti.get_entities()["task"]
                dump = 1
            if (nifti.get_entities()["datatype"] == "fmap") and (
                overwrite or "IntendedFor" not in data.keys()
            ):
